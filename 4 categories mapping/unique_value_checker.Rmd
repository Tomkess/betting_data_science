...
select distinct league, team from v_match_stats;
```

```{sql connection = con, output.var = "t_mapping_league", eval = T, include = T, comment = F, echo = F}
select tipsport_league, results_league 
from 
(select *, 
...
select tipsport_league, results_league, results_team, tipsport_team 
from 
(select *, 
...
result_teams <- 
  ...
  # - join all tipsport team to results match to see what migth be easily joined
