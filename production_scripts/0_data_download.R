...
master_data <- master_data %>%
  ...
  rowwise() %>%
  mutate(HomeTeam = ifelse("HT" %in% names(.), coalesce(HomeTeam, HT), HomeTeam),
         AwayTeam = ifelse("AT" %in% names(.), coalesce(AwayTeam, AT), AwayTeam)) %>%
  ...
  rowwise() %>%
  mutate(year_adj = ifelse(get_year %in% c("93", "94", "95", "96", "97", "98", "99"), paste("19", get_year, sep = ""), get_year)) %>%
  rowwise() %>%
  mutate(year_adj = ifelse(str_length(year_adj) < 3, paste("20", get_year, sep = ""), year_adj)) %>%
  rowwise() %>%
  mutate(created_at = as.Date(paste(get_day, get_month, year_adj, sep = "/"), format = "%d/%m/%Y")) %>%
