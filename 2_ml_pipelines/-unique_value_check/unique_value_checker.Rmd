...
upload_changes <- eventReactive(input$upload_data, {
  ...
  save(league_mapping, 
       file = paste(get("league_mapping")$mapping_storage))
})
...
Team Names
=====================================
```{r Tipsport Teams, eval = T, include = T, comment = F, echo = F}
tipsport_teams <-
  data_temp %>%
...
save(tipsport_teams, 
     file = paste(get("working_directory")[["4finance_PC"]], "/", 
                  get("data_location")$league_mapping, "/tipsport_teams.RData", 
                  sep = ""))
```
...
saved_team_changes <- reactive({
  upload_temp <- changed_team() %>% as.data.frame()
...
upload_team_changes <- eventReactive(input$upload_team_data, {
  ...
  save(team_mapping, 
       file = get("team_mapping")$mapping_storage)
})
